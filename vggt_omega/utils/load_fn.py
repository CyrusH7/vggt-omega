...
def load_and_preprocess_images(
    ...
    for image_path in image_path_list:
        # Keep geometric metadata so GT intrinsics can follow the same preprocessing.
        image = _load_rgb_image(image_path)
        original_size = image.size
        image, crop_box = _crop_to_supported_aspect_ratio_with_box(image)
    ...
    if len(shapes) > 1:
        ...
        images, padding = _pad_images_to_common_size(images, shapes, return_padding=True)
        for item, pad in zip(metadata, padding):
            item["padding"] = pad
            item["padded_shape"] = (images[0].shape[1], images[0].shape[2])
    else:
        for item in metadata:
            item["padding"] = (0, 0, 0, 0)
            item["padded_shape"] = item["resized_shape"]
...
def _crop_to_supported_aspect_ratio(image, min_aspect_ratio=0.5, max_aspect_ratio=2.0):
    return _crop_to_supported_aspect_ratio_with_box(image, min_aspect_ratio, max_aspect_ratio)[0]


def _crop_to_supported_aspect_ratio_with_box(image, min_aspect_ratio=0.5, max_aspect_ratio=2.0):
    width, height = image.size
    aspect_ratio = height / max(width, 1)
...
def _pad_images_to_common_size(images, shapes, return_padding=False):
    ...
    for image in images:
        ...
        padding.append((pad_left, pad_top, pad_right, pad_bottom))

    return (padded_images, padding) if return_padding else padded_images
